...
def test_stratified_splitter():
    ...
    splits = survhive.survival_train_test_split(X, y, rng_seed=2308, test_size=0.3)
    spl_shapes = [_.shape for _ in splits]
    assert spl_shapes == _shapes
    events = [survhive.get_indicator(_).sum() for _ in [y, splits[2], splits[3]]]
...
def test_stratified_cv_splitter():
    ...
    cv_splitter = survhive.survival_crossval_splitter(
        X, y, n_splits=3, n_repeats=2, rng_seed=2309
    )
    cv_events = [survhive.get_indicator(y[_[1]]).sum() for _ in cv_splitter]
...
def test_event_quantiles():
    ...
    assert np.array_equal(
        survhive.event_quantiles(y, quantiles=[0.25, 0.4, 0.5, 0.6, 0.75]), time_quant
    )
