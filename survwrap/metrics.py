...
def concordance_index_score(y_true, y_pred, return_all=False):
    ...
    r = sksurv.metrics.concordance_index_censored(get_indicator(y_true), get_time(y_true), y_pred)
    return r if return_all else r[0]

def _estimate_concordance_index_td(event_indicator, event_time, estimate, weights, tied_tol=1e-8):
    # taken from sksurv.metrics, generalized to a time-dependant estimate matrix (was a vector)
    # not meant to be called directly but through the concordance_index_td_scorer, since it only works if estimate has the y times as its second index
    ...
    numerator = 0.0
    denominator = 0.0
    for ind, mask, tied_time in sksurv.metrics._iter_comparable(event_indicator, event_time, order):
    ...
    if tied_time is None:
        raise sksurv.exceptions.NoComparablePairException("Data has no comparable pairs, cannot estimate concordance index.")
...
def make_time_dependent_classification_score(score_func, aggregate='mean'):
    ...
    Given a time threshold t, a survival outcome can be reduced to a classification outcome by taking as positives all events occurring before t and as negatives all events or censorings occurring after t (censorings before t are discarded as non informative).
    Using this reduction at multiple times we can extend any classification metric to survival.
...
def make_time_dependent_scorer(score_func, needs='failure', time_mode='events', time_values=None, **kwargs):
